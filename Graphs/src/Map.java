...
public class Map {
    ...
    public Map(String file) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if(lookup(row[1]) == 0)
                    cities[hash(row[1])] = new City(row[1]);
    ...
    public City lookup2 (String name){ //1 means we already added the city, 0 means we have not, -1 means theres a collision
        int hashIndex = hash(name);
        if(cities[hashIndex] == null){
            return null;}
